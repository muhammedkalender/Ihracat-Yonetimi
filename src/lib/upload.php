<?php
// http://www.eburhan.com/eburhan-upload-class-v10/
...
class UPLOAD{
    ...
    */
    function UPLOAD($_FILES2)
    {
		// GD kütüphanesi etkin mi? (geçersiz resim kontrolü için gerekli)
		$this->_gdEtkin = extension_loaded('gd');

        $dosyalar = array();
        $dosyaSay = count($_FILES2['name']);

		// yüklenecek dosya sayısına göre "dosyalar" dizisini oluştur
		if( $dosyaSay===1 && !is_array($_FILES2['name']) ) {
			$dosyalar['name'][] 	 = $_FILES2['name'];
			$dosyalar['type'][]	 	 = $_FILES2['type'];
			$dosyalar['tmp_name'][]  = $_FILES2['tmp_name'];
			$dosyalar['error'][] 	 = $_FILES2['error'];
			$dosyalar['size'][] 	 = $_FILES2['size'];
		} else {
			$dosyalar = $_FILES2;
		}
    ...
    */
	function _mimeKontrol($uzanti, $mime)
	{
		...
		$uzantilar  = array(
			...
			'ots'	=> 'application/vnd.oasis.opendocument.chart',
