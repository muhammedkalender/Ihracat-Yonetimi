...
// header("Location: /");**********************deðiþiklik yaptým OZKAN 20,12,2013 hatalarýn görünmesi için(K.adý þifre)***********
       }
   }
   ...
   function procLogin(){
	  ...
	  $lang = $_POST["lang"];
		...
		$_SESSION["lang"] = $lang;
		unset($_COOKIE['novastaihracat']); 
		setcookie("novastaihracat", $lang, time()+10800);

		if ( empty($referrer) or !isset($_POST["git"]) ) {
   ...
   function procLoginGet(){
	  ...
	  $lang = $_GET["lang"];
		...
		if ( empty($referrer) or !isset($_GET["git"]) ) {
			$referrer = $session->referrer;
		}else{
			$referrer = $referrer;
		}
   ...
   function procSendConfirm(){
	  ...
	  //username deðilde, sadece mail sorgusu
   ...
   function procHashLogin($hash){
   		...
   		if($user_info['hash_generated'] < (time() - (60*60*3))){
